...
$sql="INSERT INTO  tblusers(FullName,MobileNumber,EmailId,shopName,shopAddress,Password,Role,isActive) VALUES(:fname,:mnumber,:email,:shopname,:shopaddress,:password,:role,:isActive)";
...
$(document).ready(function(){
    $('#role').on('change', function() {
      ...
      {
        $("#shopNameAddress").show();
        $("#shopname").prop('required',true);
        $("#shopaddress").prop('required',true);
        
      }
      else
      {
        $("#shopNameAddress").hide();
        $("#shopname").prop('required',false);
        $("#shopaddress").prop('required',false);
      }
    });
});
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		...
		<input type="text" value="" placeholder="Email id" name="email" id="email" onBlur="checkAvailability()" autocomplete="off"  required>	
		 ...
		 <div style='display:none;' id='shopNameAddress'>
		 	<input type="text" value="" placeholder="Shop Name" id="shopname" name="shopname" autocomplete="off">
<textarea rows="4" cols="50" name="shopaddress" id="shopaddress" placeholder="Shop Address"></textarea>		 
		 </div>
